...
function y = AdamsMoultonL2 (f,fp,y0,dt,tend)
    ...
    for i = 2:n
        Gp = @(yn1) ( dt*7*(1-y(i-1)/10)/2 - 1);    % definiton  of the functions, those are ad hoc for a particular case
        G = @(yn1) ( y(i-1) + dt * (f(y(i-1)) + (7*(1-y(i-1)/10)*yn1)) / 2 - yn1);
        try
